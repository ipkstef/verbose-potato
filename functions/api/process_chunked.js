// Cloudflare Function for processing large CSV files (300MB+)
export async function onRequestPost(context) {
  try {
    ...
    // Check file sizes - Cloudflare limit is 50MB per file
    ...
    if (previousSize > maxSize || currentSize > maxSize) {
      return new Response(JSON.stringify({
        error: `File too large for Cloudflare Functions. Maximum size is 50MB per file. 
                Previous file: ${(previousSize / 1024 / 1024).toFixed(1)}MB, 
                Current file: ${(currentSize / 1024 / 1024).toFixed(1)}MB.
                
                For files larger than 50MB, please use the local Flask server or split your files into smaller chunks.`
      }), {
        status: 413,
        headers: { 'Content-Type': 'application/json' }
      });
    ...
    // Process the data
    const result = processPricingData(previousText, currentText);
...
function processPricingData(previousText, currentText) {
  try {
    // Parse CSV data
